...
リレーションは外部キーから推論されます。各 FK は `belongsTo`(`@relation(fields, references)`)になり、参照先モデルに逆側の `hasMany`(FK カラムが unique なら `hasOne`)が付きます。`_XToY` 形の join テーブル(2 つの FK カラムが複合 PK を構成)は暗黙 many-to-many として両側に畳み込まれ、join テーブル自体は出力から除外されます。

注意点:

- **DB に FK 制約が必要です。** リレーションは外部キーから来るので、DB が実際に FK を宣言している場合のみ復元されます。tehilim の `push` はまだ FK 制約を発行しないため、`push` で作った DB からはリレーションを復元できません(`push` での FK 発行は別途対応)。
...
- 推論されるフィールド名は機械的(参照先モデル名、複数形は末尾 `s`)なので、pull 後に適宜リネームしてください。
