...
use Polidog\Tehilim\Tests\PHPStan\Fixtures\Generated\User;

use function PHPStan\Testing\assertType;

function withoutSelect(User $client): void
{
    $row = $client->findUnique(['where' => ['id' => 1]]);
    assertType('array{id: int, email: string, name: string|null, age: int|null}|null', $row);
}

function findUniqueNarrowed(User $client): void
{
...
function findFirstNarrowed(User $client): void
{
...
function findManyNarrowed(User $client): void
{
...
function selectIncludesPkExplicitly(User $client): void
{
...
function allFalseFallsBack(User $client): void
{
...
function listFormFindUnique(User $client): void
{
...
function listFormFindMany(User $client): void
{
    $rows = $client->findMany(['select' => ['email', 'age']]);
    assertType('list<array{email: string, age: int|null, id: int}>', $rows);
}

function listFormIncludesPk(User $client): void
{
